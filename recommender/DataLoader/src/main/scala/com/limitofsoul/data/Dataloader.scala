...
object Dataloader {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val movieDF = movieRDD.map(
      item => {
        ...
        Movie(attr(0).toInt, attr(1).trim,
          attr(2).trim, attr(3).trim,
          attr(4).trim, attr(5).trim,
          attr(6).trim, attr(7).trim,
          attr(8).trim, attr(9).trim)
      }
    ).toDF()
    ...
    val tagDF = tagRDD.map(
      item => {
        val attr = item.split(",")
        Tag(attr(0).toInt, attr(1).toInt,
          attr(2).trim, attr(3).toInt)
      }
    ).toDF()
    ...
    //newTag和movie做join，数据合并在一起，左外链接
    //默认内链接
